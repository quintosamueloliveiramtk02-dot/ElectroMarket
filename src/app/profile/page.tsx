...
export default function ProfilePage({ 
  ...
  try {
    const auth = useAuth();
    hookUser = auth.user;
    hookLoading = auth.loading;
  } catch (e) {
    // Suppress useAuth exception if not inside context Provider
  }
  ...
  const fetchMyProducts = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      // Fazer a requisição para o nosso backend carregando apenas o anúncios do usuário logado
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put<any>(`/ads/${editingAd.id}`, {
        title: editTitle,
        price: parsedPrice,
        brand: editBrand,
        model: editModel,
  ...
  const handleLogoutClick = async () => {
    ...
    // 4. Redirecionar usuário de volta para a Home (/)
    if (onNavigate) {
      onNavigate('/');
    } else if (onBack) {
      onBack();
  ...
  if (!activeUser) {
    return null; // Redirect logic will handle routing
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-3 duration-250">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="border-l border-slate-200 pl-3">
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight font-sans">Perfil do Vendedor</h1>
            <p className="text-xs text-slate-500 font-medium">Gerencie suas informações e anúncios publicados</p>
          </div>
        </div>
        ...
        <button
          ...
          className="bg-red-50 hover:bg-red-105 active:bg-red-200 text-red-600 border border-red-200/60 px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition duration-205 cursor-pointer shadow-sm hover:scale-102"
        >
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col md:flex-row items-center md:items-start md:justify-between gap-6 relative overflow-hidden">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl px-5 py-4 text-center min-w-[140px] shrink-0 shadow-inner">
          ...
          <span className="text-[10px] text-slate-400 font-medium block mt-1">ativos na vitrine</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h2 className="text-lg font-bold text-slate-800 tracking-tight">Seus Anúncios Publicados</h2>
        </div>
        ...
        {!error && (
          loading ? (
            ...
            <div className="bg-white border border-slate-200 rounded-2xl py-12 px-6 text-center shadow-xs">
              ...
              <h3 className="text-base font-bold text-slate-800">Você ainda não possui anúncios publicados.</h3>
              ...
              <button
                ...
                className="mt-5 bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 rounded-xl text-sm transition active:scale-95 cursor-pointer shadow-md shadow-blue-500/10"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {products.map(ad => (
                <ProductCard 
                  ...
                  onClick={() => {
                    if (onNavigate) {
                      onNavigate(`/anuncio/${ad.id}`);
                    } else if (typeof window !== 'undefined') {
                      window.history.pushState({}, '', `/anuncio/${ad.id}`);
                      window.dispatchEvent(new PopStateEvent('popstate'));
                    }
                  }}
